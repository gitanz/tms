...
var mediaurl =  'public/lib/media-manager/';
/*---------------------------------------------------------
...
var config = (function () {
    ...
    $.ajax({
        'async': false,
        'url': mediaurl + 'scripts/filemanager.config.js',
        'dataType': "json",
        cache: false, 
...
var fileConnector = config.options.fileConnector || mediaurl + 'connectors/' + config.options.lang + '/filemanager.' + config.options.lang;
...
if($.urlParam('langCode') != 0 && file_exists (mediaurl + 'scripts/languages/'  + $.urlParam('langCode') + '.js')) config.options.culture = $.urlParam('langCode');



var lg = [];
$.ajax({
  url: mediaurl + 'scripts/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
var getFileInfo = function(file) {
	...
	$.getJSON(fileConnector + '?mode=getinfo&path=' + encodeURIComponent(file) + '&time=' + d.getMilliseconds(), function(data){
		if(data['Code'] == 0){
			...
			$('#fileinfo').find('img').attr('src', mediaurl + data['Preview']);
...
var getFolderInfo = function(path) {
	...
	$('#fileinfo').html('<img id="activity" src="' + mediaurl + 'images/wait30trans.gif" width="30" height="30" />');
		...
		if(data){
			if($('#fileinfo').data('view') == 'grid'){
				...
				for(key in data){
					...
					//alert(data[key]['Preview'].split('.').pop());
					
					var imgURL = data[key]['Preview'];
					var array = imgURL.split('&');					
					var lastSegment = array[array.length-1];
					
					if(lastSegment == 'thumbnail=true') 
						//imgURL = mediaurl + imgURL;
 

					
					result += '<li class="' + cap_classes + '" title="' + data[key]['Path'] + '"><div class="clip"><img src="' + imgURL + '" width="' + scaledWidth + '" alt="' + data[key]['Path'] + '" /></div><p>' + data[key]['Filename'] + '</p>';
...
$(function(){
	...
	if(!config.options.fileRoot) {
		fileRoot = '/' + document.location.pathname.substring(1, document.location.pathname.lastIndexOf('/') - 7) + 'uploads/';
	} else {
	...
	/** load searchbox */
	if(config.options.searchBox === true)  {
		$.getScript(mediaurl + "scripts/filemanager.liveSearch.min.js");
	} else {
		$('#search').remove();
	}
